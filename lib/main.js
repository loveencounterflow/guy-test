(function() {
  'use strict';
  var GUY, Intertype, Test, WG, _Assumptions, _jkequals, alert, create, debug, echo, help, hide, info, inspect, isa, j, log, nameit, plain, praise, reverse, rpr, to_width, type_of, urge, validate, warn, whisper;
  ...
  Test = class Test extends _Assumptions {
    ...
    _test_inner(upref, ...tests) {
      var candidate, ctx, error, i, k, key, len, len1, message, property, ref;
/* TAINT preliminary handling of arguments */
      for (i = 0, len = tests.length; i < len; i++) {
        candidate = tests[i];
        debug('Ω___6', candidate, isa.function(candidate));
      }
    ...
    _report() {
      ...
      show_totals = () => {
        ...
        whisper('Ωgt__12 ' + this.cfg.prefix, reverse(GUY.trm[color]('*'.padEnd(20), this.totals)));
      ...
      whisper('Ωgt__15 ' + this.cfg.prefix, gold('                        🙤 GUY TEST 🙦'));
      ...
      for (key in ref1) {
        ...
        whisper('Ωgt__17 ' + this.cfg.prefix, blue(key.padEnd(20), stats));
      }
      show_totals();
      repeat_totals = false;
      ref2 = this.warnings;
      for (sub_ref in ref2) {
        ...
        for (i = 0, len = messages.length; i < len; i++) {
          message = messages[i];
          whisper('Ωgt__18 ' + this.cfg.prefix, red(sub_ref), reverse(red(` ${message} `)));
        }
      }
      if (repeat_totals) {
        show_totals();
    ...
    _equals(a, b) {
      debug('Ω__19', a, b);
      debug('Ω__20', type_of(a));
      debug('Ω__21', type_of(b));
