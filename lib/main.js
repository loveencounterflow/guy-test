(function() {
  ...
  Test = class Test {
    ...
    //---------------------------------------------------------------------------------------------------------
    _report(cfg) {
      var blue, color, gold, key, message, prefix, red, ref1, ref2, stats, sub_ref;
      ...
      whisper('Ωgt___6', prefix, reverse(GUY.trm[color]('*'.padEnd(20), this.totals)));
      whisper('Ωgt___7', prefix, gold('—————————————————————————————————————————————————————————————————'));
      debug('^2243^', this.warnings);
      ref2 = this.warnings;
      for (sub_ref in ref2) {
        message = ref2[sub_ref];
...
/* TAINT preliminary handling of arguments */
      for (i = 0, len = tests.length; i < len; i++) {
        ...
        switch (true) {
          ...
          default:
            //...................................................................................................
            this._increment_fails('test', 'Ωgt__10');
      ...
      for (i = 0, len = tests.length; i < len; i++) {
        ...
        switch (true) {
          ...
          default:
            ...
            throw new Error(`Ωgt__13 expected a test, got a ${type_of(test)}`);
        }
      }
      return results;
    }
    ...
    async _throws_async(T, f, matcher) { // new Promise ( resolve, reject ) =>
      ...
      if ((ref = f.name) === '') {
        ...
        * otherwise an `error` will be caught;
         ...
         */
        throw new Error(`Ωgt__15 test method should be named, got ${rpr(f)}`);
      }
      error = null;
      try {
        //.......................................................................................................
