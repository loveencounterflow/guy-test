...
(function() {
  var ASYNC, BNP, TRM, alert, badge, call_with_timeout, debug, echo, help, info, log, njs_domain, rpr, urge, warn, whisper,
    __slice = [].slice;
  ...
  call_with_timeout = function() {
    var P, handler, keeper, keeper_id, method, test_name, timeout, _i;
    timeout = arguments[0], test_name = arguments[1], method = arguments[2], P = 5 <= arguments.length ? __slice.call(arguments, 3, _i = arguments.length - 1) : (_i = 3, []), handler = arguments[_i++];
    keeper_id = null;
    keeper = function() {
      keeper_id = null;
  ...
  module.exports = function(x, settings) {
    var check_count, fail_count, failures, new_result_handler_and_tester, pass_count, report, run, test_count;
    if (settings == null) {
      settings = null;
    }
    ...
    new_result_handler_and_tester = function(test_name) {
      ...
      RH = {};
      T = {};
      RH.success_handler = function(test_name, delta) {};
      RH.error_handler = function(delta, checked, error) {
        var entry, target;
        if (error == null) {
          delta += +1;
        }
        this.supply_caller_to_error(delta, checked, error);
        fail_count += 1;
        entry = error['caller'];
      ...
      RH.supply_caller_to_error = function(delta, checked, error) {
        var caller;
        if (error == null) {
          delta += +1;
        }
      ...
      T.eq = function() {
        ...
        if (BNP.equals.apply(BNP, P)) {
          ...
          return RH.error_handler(1, true, new Error("not equal: " + (rpr(P))));
        }
      };
      T.ok = function(result) {
        ...
        if (result === true) {
          ...
          return RH.error_handler(1, true, new Error("not OK: " + (rpr(result))));
        }
      };
      ...
      T.fail = function(message) {
        ...
        check_count += 1;
        return RH.error_handler(1, true, new Error(message));
      };
      return [RH, T];
    };
    run = function() {
            ...
            case 1:
              return tasks.push(function(handler) {
                ...
                try {
                  ...
                  error = _error;
                  RH.error_handler(0, false, error);
                }
            ...
            case 2:
              return tasks.push(function(handler) {
                ...
                return domain.run(function() {
                  ...
                  try {
                    return call_with_timeout(settings['timeout'], test_name, test, T, done);
                  } catch (_error) {
                    error = _error;
                    RH.error_handler(0, false, error);
                    whisper("completed: " + (rpr(test_name)));
