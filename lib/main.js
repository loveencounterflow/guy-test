  ...
  Assumptions = class Assumptions {
    ...
    pass(upref, cat, message = null) {
      ...
      if (this._.cfg.show_passes) {
        if (message != null) {
          message = to_width(message, this._.cfg.message_width);
    ...
    fail(upref, cat, message = null) {
      var ref;
      ref = j(this._upref, upref, 'Ωgt___3');
    ...
    eq(f, matcher) {
      ...
      ref = j(this._upref, this._._ref_from_function(f));
      try {
        //.......................................................................................................
        ...
        this.fail('Ωgt___4', 'error', message);
    ...
    //=========================================================================================================
    throws(f, matcher) {
      var error, matcher_type, shortref;
      shortref = this._._ref_from_function(f);
      error = null;
      try {
        //.......................................................................................................
        urge(j(this._upref, shortref, 'Ωgt___9'), "`throws()` result of call:", rpr(f.call(this, this)));
      } catch (error1) {
        error = error1;
        if (matcher == null) {
          this.pass(j(shortref, 'Ωgt__10'), 'error ok', error.message);
          return null;
        }
        //.....................................................................................................
        switch (matcher_type = this._._match_error(error, matcher)) {
          case true:
            this.pass(j(shortref, 'Ωgt__11'), 'error ok', error.message);
            break;
          case false:
            urge(j(this._upref, shortref, 'Ωgt__12'), "error        ", reverse(error.message));
            /* TAINT to be replaced */            warn(j(this._upref, shortref, 'Ωgt__13'), "doesn't match", reverse(rpr(matcher)));
            this./* TAINT to be replaced */fail(j(shortref, 'Ωgt__14'), 'neq', `error ${rpr(error.message)} doesn't match ${rpr(matcher)}`);
            break;
          default:
            this.fail(j(shortref, 'Ωgt__15'), 'type', `expected a regex or a text, got a ${matcher_type}`);
        }
      }
      //.......................................................................................................
      if (error == null) {
        this.fail(j(shortref, 'Ωgt__16'), 'noerr', "expected an error but none was thrown");
      }
      //.......................................................................................................
      return null;
    }
    ...
    async async_throws(f, matcher) { // new Promise ( resolve, reject ) =>
      ...
      * otherwise an `error` will be caught;
      ...
      ref = this._._ref_from_function(f);
      error = null;
      try {
        ...
        //.....................................................................................................
        if (matcher == null) {
          this.pass('Ωgt__17', 'error ok', `did throw ${rpr(error.message)}`);
          return null;
        }
        //.....................................................................................................
        switch (matcher_type = this._._match_error(error, matcher)) {
          case true:
            this.pass('Ωgt__18', 'error ok', `did throw ${rpr(error.message)}`);
            break;
          case false:
            urge(`${ref}.Ωgt__19 error        `, reverse(error.message));
            warn(`${ref}.Ωgt__20 doesn't match`, reverse(rpr(matcher)));
            this.fail('Ωgt__21', 'error nok', `did throw but not match ${rpr(error.message)}`);
            break;
          default:
            this.fail('Ωgt__22', 'fail', `expected a regex or a text for matcher, got a ${matcher_type}`);
        }
      }
      //.......................................................................................................
      if (error == null) {
        this.fail('Ωgt__23', 'missing', `expected an error but none was thrown, instead got result ${rpr(result)}`);
      }
      //.......................................................................................................
      return null;
    }
...
/* TAINT preliminary handling of arguments */
    ...
    async _async_test_inner(...tests) {
      var candidate, error, i, key, len, message, property, ref;
      for (i = 0, len = tests.length; i < len; i++) {
        candidate = tests[i];
        switch (true) {
          //.....................................................................................................
          case isa.function(candidate):
            this._test_inner(ref, candidate);
            break;
          //.....................................................................................................
    ...
    _report() {
      ...
      //.......................................................................................................
      show_totals = () => {
        whisper('Ωgt__32 ' + this.cfg.prefix, line);
        whisper('Ωgt__33 ' + this.cfg.prefix, reverse(GUY.trm[color]('*'.padEnd(20), this.totals)));
        whisper('Ωgt__34 ' + this.cfg.prefix, line);
        return null;
      };
      //.......................................................................................................
      whisper();
      whisper('Ωgt__35 ' + this.cfg.prefix, line);
      whisper('Ωgt__36 ' + this.cfg.prefix, gold('                        🙤 GUY TEST 🙦'));
      whisper('Ωgt__37 ' + this.cfg.prefix, line);
      ...
      for (key in ref1) {
        ...
        whisper('Ωgt__38 ' + this.cfg.prefix, blue(key.padEnd(20), stats));
      }
      show_totals();
      repeat_totals = false;
      ref2 = this.warnings;
      for (sub_ref in ref2) {
        ...
        for (i = 0, len = messages.length; i < len; i++) {
          message = messages[i];
          whisper('Ωgt__39 ' + this.cfg.prefix, red(sub_ref), reverse(red(` ${message} `)));
        }
      }
      if (repeat_totals) {
        show_totals();
