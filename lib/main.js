...
(function() {
  var ASYNC, CND, DIFF, alert, badge, debug, echo, help, info, log, njs_domain, rpr, urge, warn, whisper,
    slice = [].slice;
    ...
    new_result_handler_and_tester = function(test_name) {
      ...
      T.eq = function() {
        ...
        if (CND.equals.apply(CND, P)) {
          ...
          if (P.length === 2 && (CND.isa_text(p0 = P[0])) && (CND.isa_text(p1 = P[1]))) {
            info("string diff:");
            info(DIFF.colorize(rpr(p0), rpr(p1)));
            message = "not equal: " + (rpr(p0)) + ", " + (rpr(p1)) + "\nstrings not equal; see also diff above";
          } else {
